...
- **Why:** 
  ...
  - \`EventSource\` (SSE) is vastly superior to WebSockets for this use case because the communication is strictly unidirectional (Server → Client). It requires less overhead, supports native auto-reconnection, and easily bypasses corporate firewalls.
- **Alternatives considered:**
...
curl -X POST http://localhost:3001/api/v1/auth/login \\
  ...
  -d '{"storeId": "store_alpha", "storeName": "Alpha Store"}'
\`\`\`

### Example: Get Overview
\`\`\`bash
curl http://localhost:3001/api/v1/analytics/overview?period=week \\
